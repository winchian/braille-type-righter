...
* 這個公開版跟 JAWS 中文延伸套件裡的「點字鍵盤輸入」、以及隨點字顯示器發行的版本，是同一套引擎、同一組鍵位、同一個 Ctrl+F12 開關。公開版在所有點鍵都放開時送出一方；隨硬體發行的版本多一層輸入驅動，打起來更快；JAWS 版跟 JAWS 一起起落。JAWS 版在跑的時候，本程式會自動讓開，JAWS 一關就接回來。
