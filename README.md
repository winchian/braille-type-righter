# Braille Typewriter 六點打字機

本程式的用途很簡單，讓您在電腦上打六點輸入有像是打點字機的體驗。
...
* **退格**：按住空白鍵再按 D、J、K（點 2-4-5，也就是點字的 j），打錯的字就退掉；空白鍵加 D、K（點 2-5）也一樣。
* **螢幕閱讀軟體**：JAWS 在就交給 JAWS；NVDA、朗讀程式或其他螢幕閱讀軟體在，就走作業系統的 UI Automation 通知事件；什麼都沒有的時候走系統語音（SAPI），所以按下 Ctrl+F12 一定聽得到「點字鍵盤輸入已開啟」。
* 這個公開版跟 JAWS 中文延伸套件裡的「點字鍵盤輸入」、以及隨點字顯示器發行的版本，是同一套引擎、同一組鍵位、同一個 Ctrl+F12 開關。公開版在所有點鍵都放開時送出一方；隨硬體發行的版本多一層輸入驅動，打起來更快。
* **跟 JAWS 版共存**：偵測到 JAWS 在跑，本程式立刻讓開（放掉鉤子與 Ctrl+F12、不出聲、連通知視窗都收掉），由 JAWS 套件裡那一支服務；JAWS 關閉三秒後本程式自己回來，開關狀態兩邊共用。裝了 JAWS 也不會衝突。
* 點字顯示器（Focus 等）的 Perkins 鍵盤：顯示器驅動送給系統的本質上也是鍵盤按鍵；本程式目前只吃實體鍵盤的 F D S J K L A ;，顯示器的方要靠螢幕閱讀軟體的驅動轉成按鍵，尚待實機驗證。
